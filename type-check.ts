  ...
  switch (expr.tag) {
    ...
    case "builtin1":
      ...
      if (expr.name === "print") {
        ...
        if (
          !equalType(tArg.a.type, NUM) &&
          !equalType(tArg.a.type, BOOL) &&
          !equalType(tArg.a.type, NONE) &&
          !equalType(tArg.a.type, FLOAT) &&
          !equalType(tArg.a.type, { tag: "..." })
        ) {
          throw new TypeCheckError(
            SRC,
            `print() expects types "int" or "bool" or "none" as the argument, got ${bigintSafeStringify(
              tArg.a.type.tag
            )}`,
            tArg.a
          );
